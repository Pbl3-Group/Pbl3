    ...
    public class TimViecController : Controller
    {
        ...
        public async Task<IActionResult> ChiTiet(int id, string? tieuDeSeo)
        {
            ...
            if (tinTuyenDung == null)
            {
                ...
                return View("TinNotFound"); // Bạn cần tạo View TinNotFound.cshtml
            }
            ...
            {
                _logger.LogInformation("Redirecting to canonical URL for Tin ID {TinTuyenDungId}. Expected SEO: '{ExpectedSeo}', Provided SEO: '{ProvidedSeo}'", id, expectedSeoTitle, tieuDeSeo);
                return RedirectToActionPermanent("ChiTiet", new { id = id, tieuDeSeo = expectedSeoTitle });
            }

            bool daLuu = false; 
            bool daUngTuyen = false;
            bool daBaoCao = false; // Khởi tạo biến mới
            ...
            var viewModel = new ChiTietTinTuyenDungViewModel
            {
                ...
                DaBaoCao = daBaoCao, // Gán giá trị đã kiểm tra

                NguoiDangId = tinTuyenDung.NguoiDangId, 
                LoaiTaiKhoanNguoiDang = tinTuyenDung.NguoiDang?.LoaiTk ?? default, // Cung cấp giá trị mặc định nếu NguoiDang null
