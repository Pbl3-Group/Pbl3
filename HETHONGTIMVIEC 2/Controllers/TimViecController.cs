...
namespace HeThongTimViec.Controllers
{
    ...
    public class TimViecController : Controller
    {
        ...
        public async Task<IActionResult> Index([FromQuery] TimViecViewModel searchModel, int page = 1)
        {
            ...
            int pageSize = 10; // Số tin mỗi trang
           ...
           if (searchModel.LoaiLuong.HasValue && searchModel.LoaiLuong.Value != LoaiLuong.thoathuan)
            ...
            switch (searchModel.SapXep)
            {
                ...
                default: searchModel.SapXep = "ngaymoi"; query = query.OrderByDescending(t => t.TinGap).ThenByDescending(t => t.NgayDang); break;
            }
        ...
        {
            _logger.LogInformation("Xem chi tiết tin ID: {TinTuyenDungId}", id);
            int? currentUserId = GetCurrentUserId();
            ...
            {
                _logger.LogWarning("Không tìm thấy tin ID: {TinTuyenDungId}, hoặc tin chưa duyệt/đã hết hạn.", id);
                return View("TinNotFound");
            }
            ...
            bool daLuu = false; bool daUngTuyen = false;
            ...
            {
                Id = tinTuyenDung.Id, TieuDe = tinTuyenDung.TieuDe, MoTa = tinTuyenDung.MoTa, YeuCau = tinTuyenDung.YeuCau, QuyenLoi = tinTuyenDung.QuyenLoi,
                LoaiHinhCongViecDisplay = tinTuyenDung.LoaiHinhCongViec.GetDisplayName(), LoaiLuongDisplay = tinTuyenDung.LoaiLuong.GetDisplayName(),
                MucLuongDisplay = FormatSalary(tinTuyenDung.LoaiLuong, tinTuyenDung.LuongToiThieu, tinTuyenDung.LuongToiDa),
                DiaChiLamViecDayDu = diaChiLamViecDayDu, YeuCauKinhNghiemText = tinTuyenDung.YeuCauKinhNghiemText, YeuCauHocVanText = tinTuyenDung.YeuCauHocVanText,
                SoLuongTuyen = tinTuyenDung.SoLuongTuyen, TinGap = tinTuyenDung.TinGap, NgayDang = tinTuyenDung.NgayDang, NgayHetHan = tinTuyenDung.NgayHetHan,
                DaLuu = daLuu, DaUngTuyen = daUngTuyen,
                NguoiDangId = tinTuyenDung.NguoiDangId, LoaiTaiKhoanNguoiDang = tinTuyenDung.NguoiDang?.LoaiTk ?? default,
                TenNguoiDangHoacCongTy = tinTuyenDung.NguoiDang?.LoaiTk == LoaiTaiKhoan.doanhnghiep ? (tinTuyenDung.NguoiDang.HoSoDoanhNghiep?.TenCongTy ?? tinTuyenDung.NguoiDang.HoTen) : tinTuyenDung.NguoiDang?.HoTen ?? "N/A",
                LogoHoacAvatarUrl = tinTuyenDung.NguoiDang?.LoaiTk == LoaiTaiKhoan.doanhnghiep ? tinTuyenDung.NguoiDang.HoSoDoanhNghiep?.UrlLogo : tinTuyenDung.NguoiDang?.UrlAvatar,
                UrlWebsiteCongTy = tinTuyenDung.NguoiDang?.HoSoDoanhNghiep?.UrlWebsite, MoTaCongTy = tinTuyenDung.NguoiDang?.HoSoDoanhNghiep?.MoTa,
                CongTyDaXacMinh = tinTuyenDung.NguoiDang?.HoSoDoanhNghiep?.DaXacMinh ?? false,
                EmailLienHe = tinTuyenDung.NguoiDang?.Email, SdtLienHe = tinTuyenDung.NguoiDang?.Sdt, DiaChiLienHeNguoiDang = diaChiNguoiDang,
            ...
            ViewData["Title"] = $"Việc làm: {viewModel.TieuDe} tại {viewModel.TenNguoiDangHoacCongTy} | JobFlex";
